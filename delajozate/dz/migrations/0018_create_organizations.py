...
class Migration(DataMigration):
    orgModelNames = ["Stranka", "Skupina", "DrzavniZbor", "Odbor"]
    
    def forwards(self, orm):
        for mdl in [getattr(orm, n) for n in self.orgModelNames]:
            for inst in mdl.objects.all():
                if not inst.organizacija_id:
                    inst.organizacija = orm.Organizacija.objects.create()
                    inst.save()
        print "%d organizations created" % orm.Organizacija.objects.count()


    def backwards(self, orm):
        for mdl in [getattr(orm, n) for n in self.orgModelNames]:
            mdl.objects.update(organizacija=None)
        orm.Organizacija.objects.all().delete()
